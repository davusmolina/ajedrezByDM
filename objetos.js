...
function comprobarPiezaArriba(nI) {
	...
	for (var i = 0; i < 8; i++) {
		...
		if (
			alFrente < 64 &&
			isNaN(nodos[alFrente].getAttribute('id')) &&
			verificarColor(nodos[alFrente].getAttribute('id')) == verificarColor(idNi)
		) {
			return alFrente;
		} else if (
			alFrente < 64 &&
...
function comprobarPiezaAbajo(nI) {
	...
	for (var i = 0; i < 8; i++) {
		...
		if (
			abajo >= 0 &&
			isNaN(nodos[abajo].getAttribute('id')) &&
			verificarColor(nodos[abajo].getAttribute('id')) == verificarColor(idNi)
		) {
			return abajo;
		} else if (
			abajo > 7 &&
...
function comprobarPiezaIzquierda(nI) {
	...
	for (var i = 0; i < 8; i++) {
		...
		if (
			izquierda >= 0 &&
			isNaN(nodos[izquierda].getAttribute('id')) &&
			determinarFila(izquierda) == fila &&
			verificarColor(nodos[izquierda].getAttribute('id')) ==
				verificarColor(idNi)
		) {
			return izquierda;
		} else if (
...
function comprobarPiezaDerecha(nI) {
	...
	for (var i = 0; i < 8; i++) {
		...
		if (
			derecha < 64 &&
			isNaN(nodos[derecha].getAttribute('id')) &&
			determinarFila(derecha) == fila &&
			verificarColor(nodos[derecha].getAttribute('id')) == verificarColor(idNi)
		) {
			return derecha;
		} else if (
			derecha < 64 &&
...
function comprobarPiezaDiagonalAbajoIz(nI) {
	...
	for (var i = 0; i < 8; i++) {
		...
		if (
			diagonalAbajo < 64 &&
			diagonalAbajo >= 0 &&
			isNaN(nodos[diagonalAbajo].getAttribute('id')) &&
			verificarColor(nodos[diagonalAbajo].getAttribute('id')) ==
				verificarColor(nodos[nI].getAttribute('id'))
		) {
...
function casillasInfluenciaDiagonales(
	colorInfluencia,
	atributo,
	colorRey,
	llamadoPorRey,
...
function casillasInfluenciaLatVer(
	colorInfluencia,
	atributo,
	colorRey,
	llamadoPorRey,
	...
	if (llamadoPorRey == false) {
		...
		for (var i = 0; i < 8; i++) {
			...
			if (
				piezaDerechaI < 63 &&
				filaI == determinarFila(piezaDerechaI) &&
				noMasDi == 0
			) {
				piezaDerechaI += 1;
				if (isNaN(nodos[piezaDerechaI].getAttribute('id'))) {
			...
			if (
				piezaIzquierdaI > 0 &&
				filaI == determinarFila(piezaIzquierdaI) &&
				noMasIi == 0
			) {
				piezaIzquierdaI -= 1;
				if (isNaN(nodos[piezaIzquierdaI].getAttribute('id'))) {
	...
	for (var i = 0; i < 8; i++) {
		...
		if (piezaArribaF < 56 && noMasA == 0) {
			piezaArribaF += 8;
			if (
				isNaN(nodos[piezaArribaF].getAttribute('id')) &&
				nodos[piezaArribaF].getAttribute('id').includes(colorRey) == false
			) {
		...
		if (piezaAbajoF > 7 && noMasB == 0) {
			piezaAbajoF -= 8;
			if (
				isNaN(nodos[piezaAbajoF].getAttribute('id')) &&
				nodos[piezaAbajoF].getAttribute('id').includes(colorRey) == false
			) {
		...
		if (
			piezaDerechaF < 63 &&
			filaF == determinarFila(piezaDerechaF) &&
			noMasD == 0
		) {
			piezaDerechaF += 1;
			if (
				isNaN(nodos[piezaDerechaF].getAttribute('id')) &&
				nodos[piezaDerechaF].getAttribute('id').includes(colorRey) == false
			) {
		...
		if (
			piezaIzquierdaF > 0 &&
			filaF == determinarFila(piezaIzquierdaF) &&
			noMasI == 0
		) {
			piezaIzquierdaF -= 1;
			if (
				isNaN(nodos[piezaIzquierdaF].getAttribute('id')) &&
				nodos[piezaIzquierdaF].getAttribute('id').includes(colorRey) == false
			) {
...
//Añade escuchadores de evento que por cada click activan la funcion de obtenerPocision
{
	...
	nodos[1].onclick = function () {
		obtenerPocision(1);
	};
	nodos[2].onclick = function () {
		obtenerPocision(2);
	};
	nodos[3].onclick = function () {
		obtenerPocision(3);
	};
	nodos[4].onclick = function () {
		obtenerPocision(4);
	};
	nodos[5].onclick = function () {
		obtenerPocision(5);
	};
	nodos[6].onclick = function () {
		obtenerPocision(6);
	};
	nodos[7].onclick = function () {
		obtenerPocision(7);
	};
	nodos[8].onclick = function () {
		obtenerPocision(8);
	};
	nodos[9].onclick = function () {
		obtenerPocision(9);
	};
	nodos[10].onclick = function () {
		obtenerPocision(10);
	};
	nodos[11].onclick = function () {
		obtenerPocision(11);
	};
	nodos[12].onclick = function () {
		obtenerPocision(12);
	};
	nodos[13].onclick = function () {
		obtenerPocision(13);
	};
	nodos[14].onclick = function () {
		obtenerPocision(14);
	};
	nodos[15].onclick = function () {
		obtenerPocision(15);
	};
	nodos[16].onclick = function () {
		obtenerPocision(16);
	};
	nodos[17].onclick = function () {
		obtenerPocision(17);
	};
	nodos[18].onclick = function () {
		obtenerPocision(18);
	};
	nodos[19].onclick = function () {
		obtenerPocision(19);
	};
	nodos[20].onclick = function () {
		obtenerPocision(20);
	};
	nodos[21].onclick = function () {
		obtenerPocision(21);
	};
	nodos[22].onclick = function () {
		obtenerPocision(22);
	};
	nodos[23].onclick = function () {
		obtenerPocision(23);
	};
	nodos[24].onclick = function () {
		obtenerPocision(24);
	};
	nodos[25].onclick = function () {
		obtenerPocision(25);
	};
	nodos[26].onclick = function () {
		obtenerPocision(26);
	};
	nodos[27].onclick = function () {
		obtenerPocision(27);
	};
	nodos[28].onclick = function () {
		obtenerPocision(28);
	};
	nodos[29].onclick = function () {
		obtenerPocision(29);
	};
	nodos[30].onclick = function () {
		obtenerPocision(30);
	};
	nodos[31].onclick = function () {
		obtenerPocision(31);
	};
	nodos[32].onclick = function () {
		obtenerPocision(32);
	};
	nodos[33].onclick = function () {
		obtenerPocision(33);
	};
	nodos[34].onclick = function () {
		obtenerPocision(34);
	};
	nodos[35].onclick = function () {
		obtenerPocision(35);
	};
	nodos[36].onclick = function () {
		obtenerPocision(36);
	};
	nodos[37].onclick = function () {
		obtenerPocision(37);
	};
	nodos[38].onclick = function () {
		obtenerPocision(38);
	};
	nodos[39].onclick = function () {
		obtenerPocision(39);
	};
	nodos[40].onclick = function () {
		obtenerPocision(40);
	};
	nodos[41].onclick = function () {
		obtenerPocision(41);
	};
	nodos[42].onclick = function () {
		obtenerPocision(42);
	};
	nodos[43].onclick = function () {
		obtenerPocision(43);
	};
	nodos[44].onclick = function () {
		obtenerPocision(44);
	};
	nodos[45].onclick = function () {
		obtenerPocision(45);
	};
	nodos[46].onclick = function () {
		obtenerPocision(46);
	};
	nodos[47].onclick = function () {
		obtenerPocision(47);
	};
	nodos[48].onclick = function () {
		obtenerPocision(48);
	};
	nodos[49].onclick = function () {
		obtenerPocision(49);
	};
	nodos[50].onclick = function () {
		obtenerPocision(50);
	};
	nodos[51].onclick = function () {
		obtenerPocision(51);
	};
	nodos[52].onclick = function () {
		obtenerPocision(52);
	};
	nodos[53].onclick = function () {
		obtenerPocision(53);
	};
	nodos[54].onclick = function () {
		obtenerPocision(54);
	};
	nodos[55].onclick = function () {
		obtenerPocision(55);
	};
	nodos[56].onclick = function () {
		obtenerPocision(56);
	};
	nodos[57].onclick = function () {
		obtenerPocision(57);
	};
	nodos[58].onclick = function () {
		obtenerPocision(58);
	};
	nodos[59].onclick = function () {
		obtenerPocision(59);
	};
	nodos[60].onclick = function () {
		obtenerPocision(60);
	};
	nodos[61].onclick = function () {
		obtenerPocision(61);
	};
	nodos[62].onclick = function () {
		obtenerPocision(62);
	};
	nodos[63].onclick = function () {
		obtenerPocision(63);
	};
}
...
function obtenerPocision(j) {
	...
	if (seleccion == 0) {
		...
		if (
			(idPosIni == 'reyBlanco.png' && turno == 0) ||
			(idPosIni == 'reyNegro.png' && turno == 1)
		) {
			for (var i = 0; i < 64; i++) {
				if (nodos[i].getAttribute('id') == 'torreNegra.png') {
					casillasInfluenciaLatVer(
						'influenciaNegra',
						'name',
						'reyBlanco.png',
						true,
					...
					nodos[i].getAttribute('id') == 'peonNegro.png' ||
					nodos[i].getAttribute('id') == 'peonBlanco.png'
				) {
			...
			console.log(
				'la class: ' +
					nodos[posicionF].getAttribute('class') +
					' y name es: ' +
					nodos[posicionF].getAttribute('name')
			);

			if (
				idPosIni == 'reyBlanco.png' &&
				nodos[posicionF].getAttribute('name') == 'influenciaNegra'
			) {
				return;
			} else if (
				idPosIni == 'reyNegro.png' &&
				nodos[posicionF].getAttribute('class') == 'influenciaBlanca'
			) {
				return;
			} else if (
				posicionF == 62 &&
				nodos[61].getAttribute('id') == 61 &&
				nodos[62].getAttribute('id') == 62 &&
				torre.movidaBlaDer == 0 &&
				rey.movidoB == 0
			) {
				...
				posicionF == 58 &&
				nodos[58].getAttribute('id') == 58 &&
				nodos[59].getAttribute('id') == 59 &&
				nodos[57].getAttribute('id') == 57 &&
				torre.movidaBlaIzq == 0 &&
				rey.movidoB == 0
			) {
				...
				posicionF == 6 &&
				nodos[6].getAttribute('id') == 6 &&
				nodos[5].getAttribute('id') == 5 &&
				torre.movidaNegDer == 0 &&
				rey.movidoN == 0
			) {
				...
				posicionF == 2 &&
				nodos[1].getAttribute('id') == 1 &&
				nodos[2].getAttribute('id') == 2 &&
				nodos[3].getAttribute('id') == 3 &&
				torre.movidaNegIzq == 0 &&
				rey.movidoN == 0
			) {
				...
				posicionF == posicionI - 9 ||
				posicionF == posicionI - 8 ||
				posicionF == posicionI - 7 ||
			...
			(idPosIni == 'damaBlanca.png' && turno == 0) ||
			(idPosIni == 'damaNegra.png' && turno == 1)
		) {
			if (
				filaF - filaI == columnaF - columnaI ||
				filaF - filaI == columnaI - columnaF ||
				filaI - filaF == columnaI - columnaF ||
				filaI - filaF == columnaF - columnaI
			) {
				...
				if (
					range(
						comprobarPiezaDiagonalAbajoIz(posicionI),
						posicionF,
						posicionI
					) &&
					filaI > filaF &&
					...
					range(
						comprobarPiezaDiagonalAbajoDe(posicionI),
						posicionF,
						posicionI
					) &&
					filaI > filaF &&
					...
					range(
						comprobarPiezaDiagonalArribaDe(posicionI),
						posicionI,
						posicionF
					) &&
					filaF > filaI &&
					...
					range(
						comprobarPiezaDiagonalArribaIz(posicionI),
						posicionI,
						posicionF
					) &&
					filaF > filaI &&
			...
			(idPosIni == 'caballoBlanco.png' && turno == 0) ||
			(idPosIni == 'caballoNegro.png' && turno == 1)
		) {
			if (
				posicionF == posicionI - 15 ||
				posicionF == posicionI - 6 ||
				posicionF == posicionI + 10 ||
				posicionF == posicionI + 17 ||
				posicionF == posicionI + 15 ||
				posicionF == posicionI + 6 ||
				posicionF == posicionI - 10 ||
				posicionF == posicionI - 17
			...
			(idPosIni == 'alfilBlanco.png' && turno == 0) ||
			(idPosIni == 'alfilNegro.png' && turno == 1)
		) {
			if (
				filaF - filaI == columnaF - columnaI ||
				filaF - filaI == columnaI - columnaF ||
			...
			(idPosIni == 'torreBlanca.png' && turno == 0) ||
			(idPosIni == 'torreNegra.png' && turno == 1)
		) {
			if (filaF != filaI && columnaI != columnaF) {
				return;
			}
			...
			if (range(posicionI, 48, 55) && posicionF == posicionI - 16) {
				...
				posicionF == posicionI - 8 &&
				isNaN(nodos[posicionF].getAttribute('id')) == false
			) {
				peon.moverPeon(posicionI, posicionF);
			} else if (
				(posicionF == posicionI - 7 || posicionF == posicionI - 9) &&
				isNaN(nodos[posicionF].getAttribute('id'))
			) {
				peon.moverPeon(posicionI, posicionF);
			} else {
				return;
